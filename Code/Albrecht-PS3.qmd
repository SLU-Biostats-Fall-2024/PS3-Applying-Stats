...
looking at rates of tick parasitism in 5 species of Northeast bats

caught 25 bats of each species
...
This dataframe consists of 125 entries of 9 variables. scientific name(species), common_name, age, sex, and type are all categorical. tick_num is discrete whilst data.mass, body_length_mm, and tail_length_mm are continuous. \`RFe
...
There seems to be a difference between the mass of species. The hoary bat seems to me the heaviest, followed the big brown bat, silver-haired bat, little brown bat, the smallest being the eastern pipistrelle.
...
The F value is large and the p-value is small and therefore we can reject our null hypothesis that there is no difference in mass between speices.
...
There appears to be variation of body lengths across speices. The hoary bat seems to be the longest, followed by the big brown bat, silver-haired bat, little brown bat, and lastly eastern pipistrelle.
...
We want to see the exact relationships so we will conduct a pairwise comparison.
...
It looks like male bats have more ticks on them
...
Results statement: We found a signigicant difference between the mean nuber of of ticks found on the different sexes of bats, male bats having more than female ones. (df=1, F= 113.71 p= 0.000)
...
**Results statement:** We found no significant difference in number of ticks on subadult and adult bats. (df=1, F=2.6571, p=0.1056)
...
linear regression - both ratio

initial graphing relationships
...
There is a positive linear relationship between body length and tail length
...
**Results statement:** We found a significant positive correlation between body length and tail length in bats (df=1, F=1037.2, p=0.000)
